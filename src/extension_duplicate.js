...
const { Auth0AuthenticationProvider } = require("../auth-sample/authRemoteProvider.js");
...
async function activate(context) {
  ...
  //   //console.log({ octokit, githubUserInfo })
  // } catch (err) {
  //   //console.log({ err })
  // }
  ...
  //       const session = await vscode.authentication.getSession("auth0", ['api','read_user','profile'], { createIfNone: true });
  //       //console.log(session);
  //     })
  //   )
  ...
  // Authentication experiments end
  ...
  // new tree data provider experiments

  async function countAndTotalOfFilesInFolder(folderPath) {
    const result = [];
    for (const [name, type] of await vscode.workspace.fs.readDirectory(folderPath)) {
      //console.log({name,type})
      if (type === vscode.FileType.File) {
        const fileUri = vscode.Uri.file(posix.join(folderPath.path, name));
        result.push([fileUri, vscode.FileType.File]);
      } else if (type === vscode.FileType.Directory) {
        const folderUri = vscode.Uri.file(posix.join(folderPath.path, name));
        result.push([folderUri, vscode.FileType.Directory]);
      }
    }
    return result;
  }

  const resFiles = await countAndTotalOfFilesInFolder(vscode.Uri.parse(vscode.workspace.workspaceFolders[0].uri.fsPath + '/.8800'))
  const children = resFiles.map(([uri, type]) => ({ uri, type }));
  console.log({ resFiles, children })
